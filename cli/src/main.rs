...
async fn run_command() -> anyhow::Result<String> {
    ...
    match cli.command {
        CliCommand::StoreProgram {
            ...
            let program = fs::read(format!("{}/target/wasm32-unknown-unknown/release/program-noop.wasm", get_project_root()?.to_string_lossy()))?.to_vec();
            generate_types();
            let config_interface = fs::read("program-noop_serialized_config_type.txt")?;
            let aux_data_interface = fs::read("program-noop_serialized_aux_data_type.txt")?;
